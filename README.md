...
- **Deep OEM coverage: <!--OEM-COUNT-->1725<!--/OEM-COUNT--> vendor commands most tools/people don't know exist.** Dell iDRAC6/9/10, Supermicro X11/X14, AMI MegaRAC, and 9 OpenBMC vendor flavors — reverse-engineered from firmware and Ghidra, callable by name, not just raw opcodes.
...
- **Readable wire traces.** Color-coded `-d` output showing the full request/response back-and-forth with every packet field labeled — "ipmi-hex friendly," about as friendly as hex gets.
